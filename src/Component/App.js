...
class App extends React.Component {
    ...
    onSubmitDeleteModal = () => {
        ...
        let updateVlRows = this.state.vlrows.filter((currValue) => {
            if (currValue.rwid !== this.state.deleteModalBody.rwid) {
                ...
                console.log("delete",currValue.name);
            }
        });
        console.log("updateVlRows",updateVlRows);
    ...
    onSearchSubmit = (resultRow, term) => {
        if(term.length>0){
            this.setState({ 'vlrows': resultRow });

        }else{
            
            this.setState({ 'vlrows': this.state.vlrowsLocal });
